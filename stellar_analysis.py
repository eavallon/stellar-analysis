...
def get_column_data(path, column_name):
    ...
    Returns: 
        A dictionary in which the key is the id number of each star and the 
...
def get_coordinates(ra, dec):
    """
    Given a coordinate, returns a dictionary mapping the star's id to both its 
    coordinate. 
    
    Parameters:
        ra: a dictionary mapping a star's id to its Right Acension 
            (similar to longitude).
            
        dec: a dictionary mapping a star's id to its Declination (similar to 
             latitude).
    
    Returns: 
       A dictionary in which the key is the star's id number and value is an 
...
def color_magnitude_plot(magnitude, temperature):
    ...
    Parameters:
        magnitude: a dictionary in which the keys are star id numbers and the 
                   values are the Kepler magnitude of the star.
            
        temperature: a dictionary in which the keys are star id numbers and the 
                     values are the E(B-V) of the star.
...
def get_star_type(magnitude, temperature):
    ...
    are either Kepler Magnitude or the Temperature, returns a dictionary of star
    types
...
def h_m_s_separator(coordinate):
    """
    Converts a coordiate of a star as a string to a list of values. 
    
    Parameters:
        coordinate: a string corresponding to a star's location in 
...
def check_if_coords_close(coord1, coord2, coord_range):
    """
    Checks if the first star/coordinate given is within the range of the second 
    star/coordinate.
    
    Parameters:
        coord1: a list that represents one coordinate of a star.
        
        coord2: a list that represents one coordinate of another star.
...
def find_surrounding_stars(kic, coord, ra, dec):
    """
    Given a star's id number and its coordinate, returns a list of stars that 
    are close to the given star.

    Parameters:
        kic: an integer value representing a star's id number.
...
def percent_list_in_list(list1, list2):
    """
    Given two lists, returns the percentage of values that are in both lists.
    ...
    Returns: 
        A float representing the percentage of values in both lists.
    """
    total = len(list2)
...
def near_stars_same_type_percentage(type_dict, ra, dec):
    """
    Given a dictionary of star types, checks to see if stars that are near each 
    other are of the same type.
    
    Parameters:
        
        type_dict: a dictionary in which the keys are strings corresponding to 
                   the type of star, and the values are lists of star id numbers.
    ...
    Returns: 
        ...
        floats representing the percentage of same-type stars in the surrounding 
        area.
    """
    out_percents_dict = {}
...
def plot_near_stars_same_type_histogram(data, type_dict):
    ...
    Parameters:
        data: a dictionary in which the keys are star id numbers, and the values
              are percentages.
...
def main():
    ...
    get_star_type(magnitude, temperature)
